...
651 papers, organized by mechanism instead of by year.
...
![Entries](https://img.shields.io/badge/entries-651-blue.svg?style=flat-square)
...
- [Graph Neural Networks and Graph Processing](#graph-neural-networks-and-graph-processing)
  ...
  - [Dynamic, temporal and non-homogeneous GNN accelerators](#dynamic-temporal-and-non-homogeneous-gnn-accelerators)
  ...
  - [Large-scale GNN training systems](#large-scale-gnn-training-systems)
...
| [ESE: Efficient Speech Recognition Engine with Sparse LSTM on FPGA](https://doi.org/10.1145/3020078.3021745) | FPGA'17 | Load-balance-aware pruning plus a scheduler that partitions the compressed LSTM across PEs to keep sparse matrix-vector work even. | — |
...
| [PermDNN: Efficient Compressed DNN Architecture with Permuted Diagonal Matrices](https://doi.org/10.1109/MICRO.2018.00024) | MICRO'18 | Permuted-diagonal weight blocks give index-free structured sparsity, executed on a multiplier array without FFT or gather logic. | — |
...
| [MASR: A Modular Accelerator for Sparse RNNs](https://doi.org/10.1109/PACT.2019.00009) | PACT'19 | Bitmask-encoded dynamic activations and static weights are intersected per timestep, with modular banks sized to the activation footprint. | — |
...
| [SqueezeFlow: A Sparse CNN Accelerator Exploiting Concise Convolution Rules](https://doi.org/10.1109/TC.2019.2924215) | TC'19 | PT-OS-sparse dataflow decomposes sparse convolution into concise rules, removing ineffectual work without modifying the dense PE datapath. | — |
...
| [SparseTrain: Exploiting Dataflow Sparsity for Efficient Convolutional Neural Networks Training](https://doi.org/10.1109/DAC18072.2020.9218710) | DAC'20 | Stochastic pruning of activation gradients feeds a 1D-convolution dataflow that skips both natural ReLU zeros and induced zeros. | — |
...
| [Bootes: Boosting the Efficiency of Sparse Accelerators Using Spectral Clustering](https://doi.org/10.1145/3725843.3756125) | MICRO'25 | Spectral clustering on a row-similarity matrix reorders A so row-wise product reuses B fibers, with a skip test for unhelpful patterns. | — |
...
| Paper | Venue | Contribution | Code |
|---|---|---|---|
| [MAERI: Enabling Flexible Dataflow Mapping over DNN Accelerators via Reconfigurable Interconnects](https://doi.org/10.1145/3173162.3173176) | ASPLOS'18 | Tiny-switch reconfigurable distribution and augmented reduction trees map arbitrary dense or sparse dataflows onto non-contiguous PE groups. | — |
...
| [ALRESCHA: A Lightweight Reconfigurable Sparse-Computation Accelerator](https://doi.org/10.1109/HPCA47549.2020.00029) | HPCA'20 | Mathematically reorders sparse kernels into independent and data-dependent parts, switching a fixed compute unit against a reconfigurable reduction tree. | — |
| [ASCELLA: Accelerating Sparse Computation by Enabling Stream Accesses to Memory](https://doi.org/10.23919/DATE48585.2020.9116501) | DATE'20 | Streams nonzero rows into parallel compute lanes behind a fast on-the-fly decompression mechanism, removing indirect-access stalls. | — |
...
| [Capstan: A Vector RDA for Sparsity](https://doi.org/10.1145/3466752.3480047) | MICRO'21 | Reconfigurable dataflow array with format-independent sparse iteration and memory primitives, plus out-of-order random-access SRAM banking. | — |
...
| [FlexNeRFer: A Multi-Dataflow, Adaptive Sparsity-Aware Accelerator for On-Device NeRF Rendering](https://doi.org/10.1145/3695053.3731107) | ISCA'25 | Flexible NoC over a precision-scalable MAC array, selecting the sparse storage format from the measured sparsity ratio and precision mode. | — |

### Dual-sided sparsity and sparsity-degree versatility

| Paper | Venue | Contribution | Code |
|---|---|---|---|
| [STICKER: An Energy-Efficient Multi-Sparsity Compatible Accelerator for Convolutional Neural Networks in 65-nm CMOS](https://doi.org/10.1109/JSSC.2019.2946771) | JSSC'20 | Nine sparsity modes switch the datapath, a reconfigurable hierarchical data memory, and a set-associative PE array matching both operand densities. | — |
...
| [TaskFusion: An Efficient Transfer Learning Architecture with Dual Delta Sparsity for Multi-Task Natural Language Processing](https://doi.org/10.1145/3579371.3589040) | ISCA'23 | Reuses activations and weights across tasks sharing one input, exploiting delta sparsity in both on a heterogeneous multi-task datapath. | — |
...
| [GUST: Graph Edge-Coloring Utilization for Accelerating Sparse Matrix Vector Multiplication](https://doi.org/10.1145/3622781.3674186) | ASPLOS'24 | Separates multipliers from adders so rows and columns share both, resolving the resulting collisions by edge-coloring the access graph. | — |
...
| [Acamar: A Dynamically Reconfigurable Scientific Computing Accelerator for Robust Convergence and Minimal Resource Underutilization](https://doi.org/10.1109/MICRO61859.2024.00117) | MICRO'24 | Dynamically reconfigurable datapath for sparse scientific solvers, adapting to per-iteration sparsity variation instead of committing to one static design. | — |
...
| [Pipirima: Predicting Patterns in Sparsity to Accelerate Matrix Algebra](https://doi.org/10.1109/DAC63849.2025.11132628) | DAC'25 | Branch-predictor-style saturating counters anticipate nonzero locations while streaming CSR matrices, using the prediction to relieve load imbalance. | — |
...
| [Telos: A Dataflow Accelerator for Sparse Triangular Solver of Partial Differential Equations](https://doi.org/10.1145/3695053.3731078) | ISCA'25 | Plane-parallel pipelining maps stencil-structured triangular solves onto PEs, with cross-plane communication aggregated into systolic transfers. | — |
| [Transitive Array: An Efficient GEMM Accelerator with Result Reuse](https://doi.org/10.1145/3695053.3731043) | ISCA'25 | Transitive sparsity: reuses previously computed results as GEMM partial sums, ordering execution over a directed acyclic graph, multiplication-free. | — |
| [Cambricon-SR: An Accelerator for Neural Scene Representation with Sparse Encoding Table](https://doi.org/10.1145/3695053.3731018) | ISCA'25 | Prunes the neural-scene encoding table and walks its sparsity bitmap with a sequential-SRAM sparse index unit to drop invalid fetches. | — |
| [Misam: Machine Learning Assisted Dataflow Selection in Accelerators for Sparse Matrix Multiplication](https://doi.org/10.1145/3725843.3756126) | MICRO'25 | Decision-tree and reinforcement-learning models pick inner, outer, or row-wise dataflow per matrix and reconfigure the accelerator accordingly. | — |
...
| [The Sparsity-Aware LazyGPU Architecture](https://doi.org/10.1145/3695053.3731009) | ISCA'25 | Issues memory requests lazily and elides them when the fetched values are zero, with a zero cache killing dead wavefront requests. | — |
| [SegFold: Accelerating Sparse GEMM with a Fine-Grained Dynamic Dataflow](https://doi.org/10.1109/ISCA66397.2026.00130) | ISCA'26 | Segment dataflow: dynamic work assignment over a local reuse window plus a merge network that remaps partially completed work. | — |
| [TensorPrism: Rethinking Sparse High-Order Tensor Acceleration via Co-Occurrence Graph](https://doi.org/10.1109/ISCA66397.2026.00035) | ISCA'26 | Transforms a high-order tensor into a co-occurrence graph capturing nonzero correlations across all modes, avoiding matricization's loss of reuse. | — |
| [Harmonia: A Unified Hierarchical Scheduling Framework for Sparse Matrix Multiplication](https://doi.org/10.1109/ISCA66397.2026.00151) | ISCA'26 | Offline model fixes initial tiling and dataflow mapping; runtime sparsity-pattern detection reselects tile shape and dataflow mode per region. | — |
| [HiT: A Unified Sparsity-Adaptive Architecture for High-Throughput Matrix Multiplication](https://doi.org/10.1109/ISCA66397.2026.00187) | ISCA'26 | Two outer-product dataflows for high and moderate sparsity plus an inner-product dense mode, fed by a parallel intersection and distribution unit. | — |
| [MLX: Multi-Layer Execution for Structured LLM Workload Acceleration on Spatial Architectures](https://doi.org/10.1109/ISCA66397.2026.00017) | ISCA'26 | Folds butterfly-structured sparse projections into closed dependency components pipelined across layers on a spatial mesh with bounded-hop skip routing. | — |

> **Design-space note.** The three SpGEMM dataflows differ mainly in what each is forced to keep on chip. Inner product computes every output exactly once, so it needs no merge network and no partial-matrix storage, but below roughly 1% density almost every fiber intersection misses and the multipliers idle. Outer product never performs a useless multiply and gets perfect input reuse, but it emits O(nnz) partial matrices whose merge dominates DRAM traffic the moment they spill. Gustavson (row-wise) product is the compromise that won: it also skips all zero work, yet it confines merging to one output row, so the merge state is bounded by a row rather than a matrix and fits in a scratchpad, which is exactly what Gamma's FiberCache, MatRaptor's queues, and NeuraChip's hash tables are. What remains is that a row's working set is data-dependent, which is why the 2023-2026 line of work is about adaptivity rather than a new product form: Spada's variable window, Flexagon's merger-reduction network, Trapezoid's density-driven switch between inner-product and Gustavson, HYTE's hybrid tiling, SegFold's fully dynamic per-segment scheduling, Harmonia's two-level offline-then-runtime dataflow selection, and HiT's split between two outer-product modes and a dense inner-product mode. The one recent result that does not fit that framing is Transitive Array, which stops asking which zeros to skip and instead reuses an already computed product as the starting point of the next one, so the saving comes from redundancy between outputs rather than from zeros in the inputs. Analytical models (Sparseloop, TeAAL) exist because enumerating those choices is now cheaper than taping out each point.
...
| [EGMA: Enhancing Data Reuse and Workload Balancing in Message Passing GNN Acceleration via Gram Matrix Optimization](https://doi.org/10.1145/3649329.3655962) | DAC'24 | Uses Gram-matrix structure of the message-passing graph to expose data reuse and balance per-vertex work. | — |
| [CoCoA: Algorithm-Hardware Co-Design for Large-Scale GNN Training using Compressed Graph](https://doi.org/10.1145/3676536.3676835) | ICCAD'24 | Compresses the graph by homophily-guided node merging and embedding vector quantization, with an accelerator reading the compressed form. | — |
...
| [SiGNoR: Similarity-based Graph Partitioning and Node Reuse for Memory Efficient GNN Acceleration](https://doi.org/10.1109/ICCAD66269.2025.11240946) | ICCAD'25 | Replaces halo nodes with similar local nodes inside each partition, plus partition-wise block floating-point for feature storage. | — |
...
### Dynamic, temporal and non-homogeneous GNN accelerators

| Paper | Venue | Contribution | Code |
|---|---|---|---|
| [E-DGCN: An Efficient Hardware Accelerator Design for Dynamic Graph Convolutional Network (DGCN) Inference](https://doi.org/10.1145/3649329.3658254) | DAC'24 | Reconfigurable processing elements cover both the GCN sparse-dense products and the RNN element-wise and Hadamard operators. | — |
| [CDA-GNN: A Chain-driven Accelerator for Efficient Asynchronous Graph Neural Network](https://doi.org/10.1145/3649329.3656240) | DAC'24 | Chain-driven asynchronous execution regularizes state propagation along vertex dependency chains, with chain-aware caching for locality. | — |
| [RTGA: A Redundancy-free Accelerator for High-Performance Temporal Graph Neural Network Inference](https://doi.org/10.1145/3649329.3656241) | DAC'24 | Temporal-tree execution drops repeated sampling and refetching of vertex memory across snapshots, with temporal-aware feature caching. | — |
| [MeHyper: Accelerating Hypergraph Neural Networks by Exploring Implicit Dataflows](https://doi.org/10.1109/HPCA61900.2025.00073) | HPCA'25 | Microedge-centric dataflow reuses information shared between hyperedge and vertex aggregation tasks, with hierarchical feature management. | — |
| [Cambricon-DG: An Accelerator for Redundant-Free Dynamic Graph Neural Networks Based on Nonlinear Isolation](https://doi.org/10.1109/HPCA61900.2025.00074) | HPCA'25 | Nonlinear isolation makes incremental snapshot processing exact, paired with vertex-wise RNN computation and a topology management engine. | — |
| [I-DGNN: A Graph Dissimilarity-based Framework for Designing Scalable and Efficient DGNN Accelerators](https://doi.org/10.1109/HPCA61900.2025.00081) | HPCA'25 | Decouples inter-snapshot dissimilarity computation from the layer-by-layer pipeline so intermediate data stops spilling off-chip. | — |
| [DiTile-DGNN: An Efficient Accelerator for Distributed Dynamic Graph Neural Network Inference](https://doi.org/10.1145/3695053.3731017) | ISCA'25 | Redundancy-free parallelism across snapshots plus workload-balance optimization on a reconfigurable array covering both GNN and RNN kernels. | — |
| [HEAT: NPU-NDP Heterogeneous Architecture for Transformer-Empowered Graph Neural Networks](https://doi.org/10.1145/3725843.3756117) | MICRO'25 | Splits transformer-empowered GNNs across an NPU and DIMM-based near-data processing, assigning vertices by topology-derived importance. | — |
...
| [GraphAGILE: An FPGA-Based Overlay Accelerator for Low-Latency GNN Inference](https://arxiv.org/abs/2302.01769) | TPDS'23 | Overlay with an instruction set and adaptive computation kernel covering GEMM, SpDMM and SDDMM, so no reconfiguration per model. | — |
...
| [GCV-Turbo: End-to-end Acceleration of GNN-based Computer Vision Tasks on FPGA](https://arxiv.org/abs/2404.07188) | FCCM'24 | One resource pool serves both CNN and GNN kernels, with a compiler optimizing the mixed computation graph end to end. | — |
...
| [GoPIM: GCN-Oriented Pipeline Optimization for PIM Accelerators](https://doi.org/10.1109/HPCA61900.2025.00056) | HPCA'25 | Allocates ReRAM crossbar resources per pipeline stage and interleaves vertex updates to even out skewed degrees across crossbars. | — |
| [EOD: Enabling Low Latency GNN Inference via Near-Memory Concatenate Aggregation](https://doi.org/10.1145/3695053.3731083) | ISCA'25 | Near-memory units build the computational graph and aggregate features in place, removing host-side preparation from inductive serving. | — |
| [Anchor First, Accelerate Next: Revolutionizing GNNs with PIM by Harnessing Stationary Data](https://doi.org/10.1109/DAC63849.2025.11132411) | DAC'25 | Near-bank PIM with a partitioner maximizing stationary vertices and a dataflow that accesses non-stationary data exactly once. | — |
| [3D-SubG: A 3D Stacked Hybrid Processing Near/In-Memory Accelerator for Subgraph GNNs](https://doi.org/10.1109/DAC63849.2025.11132830) | DAC'25 | Logic die of SRAM PIM macros bonded to DRAM, with bit-level nonzero gathering and balanced mapping of independent subgraphs. | — |
...
| [Accel-GCN: High-Performance GPU Accelerator Design for Graph Convolution Networks](https://arxiv.org/abs/2308.11825) | ICCAD'23 | Degree sorting plus block-level partitioning that resizes warp workloads, with a combined-warp strategy for coalesced dense-column access. | [code](https://github.com/xiexi51/ICCAD-Accel-GCN) |
...
| [Hector: An Efficient Programming and Compilation Framework for Implementing Relational Graph Neural Networks in GPU Architectures](https://arxiv.org/abs/2301.06284) | ASPLOS'24 | Two-level IR for relational GNNs that decouples model semantics, data layout and operator optimization, generating kernels from reusable templates. | [code](https://github.com/K-Wu/HET) |
...
### Large-scale GNN training systems
...
| [Mithril: A Scalable System for Deep GNN Training](https://doi.org/10.1109/HPCA61900.2025.00082) | HPCA'25 | Layer-level model parallelism over the whole graph, breaking chunk dependencies with embedding speculation instead of partitioning the graph. | — |
| [Buffalo: Enabling Large-Scale GNN Training via Memory-Efficient Bucketization](https://doi.org/10.1109/HPCA61900.2025.00083) | HPCA'25 | Bucketizes nodes by degree for flexible node-to-partition mapping, with analytical memory modeling and sampling-based block generation. | — |
| [VeloxGNN: Efficient Out-of-Core GNN Training with Delayed Gradient Propagation](https://doi.org/10.1109/HPCA68181.2026.11408524) | HPCA'26 | Delayed gradient propagation over historical embeddings so an out-of-core epoch reads each storage-resident partition at most once. | — |
| [PULSE: Efficient Temporal Graph Network Training via Unified Redundancy Elimination](https://doi.org/10.1145/3779212.3790157) | ASPLOS'26 | Minimal input, storage and reuse units deduplicate temporal-graph computation, reconstruct messages on demand, and pool GPU memory blocks. | — |
...
| [Early-Bird GCNs: Graph-Network Co-Optimization Towards More Efficient GCN Training and Inference via Drawing Early-Bird Lottery Tickets](https://arxiv.org/abs/2103.00794) | AAAI'22 | Detector spots graph lottery tickets early in training, then co-sparsifies adjacency and weights from that point on. | [code](https://github.com/GATECH-EIC/Early-Bird-GCN) |
| [GCond: Graph Condensation for Graph Neural Networks](https://arxiv.org/abs/2110.07580) | ICLR'22 | Synthesizes a tiny graph by matching GNN training gradients, condensing node features and structure simultaneously. | [code](https://github.com/ChandlerBang/GCond) |
...
| [GraphPIM: Enabling Instruction-Level PIM Offloading in Graph Computing Frameworks](https://doi.org/10.1109/HPCA.2017.54) | HPCA'17 | Offloads graph atomic operations to HMC-side PIM instructions, with hardware and software mechanisms for routing eligible atomics. | — |
...
| [GraphPulse: An Event-Driven Hardware Accelerator for Asynchronous Graph Processing](https://doi.org/10.1109/MICRO50266.2020.00078) | MICRO'20 | Event-driven dataflow carries vertex values inside the events themselves, coalescing in-flight events instead of tracking active sets. | — |
...
| [MEGA Evolving Graph Accelerator](https://doi.org/10.1145/3613424.3614260) | MICRO'23 | Streams edge additions as incremental events and runs snapshots sharing update batches concurrently, packing and pipelining compatible batches. | — |
...
| [NOVA: A Novel Vertex Management Architecture for Scalable Graph Processing](https://doi.org/10.1109/HPCA61900.2025.00072) | HPCA'25 | Decouples reduction from propagation and keeps the working set off-chip, using on-chip memory as a latency-hiding buffer. | — |
| [SparseWeaver: Converting Sparse Operations as Dense Operations on GPUs for Graph Workloads](https://doi.org/10.1109/HPCA61900.2025.00108) | HPCA'25 | GPU functional unit that rewrites sparse graph operations into dense ones using topology, balancing work across warp threads. | — |
| [FALA: Locality-Aware PIM-Host Cooperation for Graph Processing with Fine-Grained Column Access](https://doi.org/10.1145/3725843.3756127) | MICRO'25 | Fine-grained column access inside DRAM banks lets PIM and host cache split vertices by reuse instead of overfetching. | — |
| [X-SET: An Efficient Graph Pattern Matching Accelerator With Order-Aware Parallel Intersection Units](https://doi.org/10.1145/3725843.3756112) | MICRO'25 | Order-aware set-intersection units exploit sorted adjacency to cut comparator count, with a barrier-free out-of-order search-tree scheduler. | — |
| [RidgeWalker: Perfectly Pipelined Graph Random Walks on FPGAs](https://arxiv.org/abs/2601.11057) | HPCA'26 | Markov property lets walks decompose into stateless out-of-order tasks, driven by a queuing-theory feedback scheduler. | — |
...
| [GPUs All Grown-Up: Fully Device-Driven SpMV Using GPU Work Graphs](https://doi.org/10.1145/3695053.3731060) | ISCA'25 | Fuses per-row algorithm selection with the row processing itself via GPU work graphs, removing the host round trip. | — |
...
| [pSyncPIM: Partially Synchronous Execution of Sparse Matrix Operations for All-Bank PIM Architectures](https://doi.org/10.1109/ISCA59077.2024.00034) | ISCA'24 | Lets each bank's unit diverge from all-bank lock-step, so random sparsity no longer stalls commercial PIM. | — |
...
| [Phi: Leveraging Pattern-based Hierarchical Sparsity for High-Efficiency Spiking Neural Networks](https://doi.org/10.1145/3695053.3731035) | ISCA'25 | Splits spike activations into precomputed vector patterns plus a sparse residual, exploiting their distribution rather than only skipping zeros. | — |
...
| [ParetoES: Hardware-Accelerated Sparse Embedding Similarity via Pareto-Optimal Pruning](https://doi.org/10.1109/ISCA66397.2026.00131) | ISCA'26 | Treats sparse embedding retrieval as memory-bound top-K SpMV, pruning the candidate space by clustering and quantization on FPGA. | — |
